...
# run miniprot
subprocess.call('miniprot -Iut16 -t ' + threads + ' --trans ' + genome + ' ' + proteins + ' -T ' + gc + ' > '+genome.split('.')[0]+'.'+proteins.split('.')[0]+'.miniprot',shell=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
p = 1
while n < len(mini):
    out.write('>'+genome.split('.')[0]+'.seq_'+str(p)+'\n'+mini[n+1].strip().split('\t')[1]+'\n')
    n += 2
    p += 1
out.close()

# cluster the resulting sequences at 100% identity
subprocess.call('cd-hit -i '+genome.split('.')[0]+'.'+proteins.split('.')[0]+'.miniprot.fasta'+' -o ' + genome.split('.')[0]+'.'+proteins.split('.')[0]+'.miniprot.fasta.100c' + ' -c 1.0 -T '+threads,shell=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
